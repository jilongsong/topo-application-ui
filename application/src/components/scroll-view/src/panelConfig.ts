...
const property = (): PanelConfig<MScrollView['property']> => {
  return {
    fields: {
      overflowX: {
        type: FieldType.Enum,
...
const style: PanelConfig<MScrollView['style']> = {
  fields: {
    ...baseStyleConfig(),
  },
};

const event = (services: Services): PanelConfig<MScrollView['event']> => {
  return {
    fields: {
      onSelect: {
