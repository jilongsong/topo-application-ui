...
import { getHost, injectStyle, isSameDomain } from '@topo/utils';
...
export default class StageRender extends EventEmitter {
  ...
  public gettopoApi = (): topo => ({
    onRendererPending: () => {
      this.isRendered = false;
      this.emit('renderer-pending');
    },
    onRendererResolve: (el: HTMLElement) => {
      this.isRendered = true;
      this.emit('renderer-resolve', el);
    },
  ...
  private posttopoRuntimeReady() {
    this.contentWindow = this.iframe?.contentWindow as RuntimeWindow;

    this.contentWindow.topo = this.gettopoApi();

    this.contentWindow.postMessage(
      {
        topoRuntimeReady: true,
      },
      '*'
    );
  }
