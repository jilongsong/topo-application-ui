...
import StageCore, { GuidesType, SortEventData, UpdateEventData } from '@topo/stage';
  ...
  stage.on('update', (ev: UpdateEventData) => {
    if (ev.parentEl) {
      for (const data of ev.data) {
        editorService.moveToContainer({ id: data.el.id, style: data.style } as any, ev.parentEl.id);
      }
      return;
    }

    // TODO: 需要解决类型不匹配问题
    editorService.update(ev.data.map((data) => ({ id: data.el.id, style: data.style })) as any);
  });
