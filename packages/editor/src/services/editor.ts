...
import type { Id, MApp, MComponent, MContainer, MNode, MPage } from '@topo/schema';
import StageCore from '@topo/stage';
import { getNodePath, isNumber, isPage, isPop } from '@topo/utils';
...
export class EditorService extends BaseService {
  ...
  public async getApp(): Promise<Core | undefined | null> {
    ...
    if (node?.id) {
      this.get<StageCore>('stage')
        ...
        ?.emit('editor:select', {
          ...
          page,
          parent,
          path: getNodePath(node.id, this.get<MApp>('root').items),
        });
    }
  ...
  public async selectNextNode(): Promise<MNode> | never {
    ...
    if (!node || isPage(node) || node.type === 'app') return node;
  ...
  public async doAdd(node: MNode, parent: MContainer): Promise<MNode> {
    ...
    if ((parent?.type === 'app' || curNode.type === 'app') && node.type !== 'page') {
      throw new Error('app下不能添加组件');
    }

    if (parent.id !== curNode.id && node.type !== 'page') {
  ...
  public async moveToContainer(config: MNode, targetId: Id): Promise<MNode | undefined> {
    ...
    if (node && parent && stage) {
      ...
      this.emit('moveToContainer', newConfig);
